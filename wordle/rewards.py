...
def output_format_check(completions, **kwargs): 

  def _output_format_check(completion):
    reward = 0
    try:
      ...
      word_list = pd.read_csv(WORD_LIST_URL)
  ...
  print(completions[0])
  rewards = [_output_format_check(completion) for completion in completions]
  print("output_format_check rewards: ", rewards)
  return rewards
...
def uses_previous_feedback(completions, **kwargs):

  def _uses_previous_feedback(completion, past_guess_history):
    reward = 0
    try:
      completion = "<think>" + completion
  ...
  rewards = [_uses_previous_feedback(c, pgh) for c, pgh in zip(completions, kwargs["past_guess_history"])]
  print("uses_previous_feedback rewards: ", rewards)
  return rewards
...
def guess_value(completions, **kwargs):

  def _guess_value(completion, past_guess_history):
    reward = 0
    try:
      completion = "<think>" + completion
      regex = r"<guess>\s*([\s\S]*?)\s*<\/guess>$"
  ...
  rewards = [_guess_value(c, pgh) for c, pgh in zip(completions, kwargs["past_guess_history"])]
  print("guess_value rewards: ", rewards)
  return rewards
